...
from .custom_relu import C_ReLU
...
class ResNetBasicblock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        self.conv1 = linear_conv3x3(inplanes, planes, stride)
        self.bn1 = nn.BatchNorm2d(expand_ratio * planes)
        self.relu = nn.ReLU(inplace=True)
        self.train_act = C_ReLU()
        self.eval_act = nn.ReLU(inplace=True)
        self.conv2 = nn.Conv2d(expand_ratio * planes, planes, 3, 1, 1, bias=False)
    ...
    def forward(self, x):
        ...
        out = self.bn1(out)
        if self.training:
            out = self.train_act(out)
        else:
            out = self.eval_act(out)
